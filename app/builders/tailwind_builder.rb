...
class TailwindBuilder < ActionView::Helpers::FormBuilder
  ...
  def submit(attribute, options = {})
    options[:class] = "text-primary-foreground font-bold py-2 px-4 rounded #{options[:class]}".strip
    if options[:disabled]
      # color muted
      options[:class] = "bg-gray-500 text-white cursor-not-allowed #{options[:class]}".strip
    else
      options[:class] = "bg-primary hover:bg-primary-darker cursor-pointer #{options[:class]}".strip
    end
